...
export interface CaseStudyCardHandle {
   ...
   *  and registers one observer that calls every card's playExit(). */
  playExit: () => void;
}

export const CaseStudyCard = forwardRef<CaseStudyCardHandle, Props>(function CaseStudyCard(
  { study, index, onExitStart },
  ref,
) {
  ...
  useLayoutEffect(() => {
    ...
    const reveal = createTextReveal(titleEl, { duration: 0.75, stagger: 0.5 });
  ...
  return (
    ...
    >
      <div ref={mediaRef} className="case-card__media">
        ...
        <canvas ref={canvasRef} className="case-card__canvas" aria-hidden="true" />
      </div>
